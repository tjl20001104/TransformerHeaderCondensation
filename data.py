...
class MyDataSet(Data.Dataset):
    ...
    def padding_batch(self, batch):
        decoder_inputs = torch.tensor(np.array([d["decoder_input"] for d in batch]), dtype=torch.long)
        decoder_outputs = torch.tensor(np.array([d["decoder_output"] for d in batch]), dtype=torch.long)
...
def generate_sequence(args, dataset, mode=1, **kwargs):
    ...
    # 大一统任务
    elif args.target == 'unified':
        seq = tasks_unified(args, seq, dataset, mode=mode)

    # x3 x34 x345长度随机任务
    elif args.target == 'multi_composition':
        seq = tasks_multi_composition(args, seq, dataset, mode=mode)
...
def get_train_data(args, return_dict=False, **kwargs):
    ...
    if kwargs and 'use_mod_list_specific' in kwargs and bool(kwargs['use_mod_list_specific']):
        variable_train_lst, _ = generate_mod_list_specific(args.data_min, args.data_max, args.seq_len-1)
    else:
        variable_train_lst, _ = generate_mod_list(args.data_min, args.data_max, args.seq_len-1)
    ...
    train_data_loader = Data.DataLoader(train_dataset, shuffle=True, batch_size=args.train_batch_size, 
...
def get_test_data(args, return_dict=False, **kwargs):
    ...
    if args.test_data_size == 0:
        ...
        test_data_loader = Data.DataLoader(test_dataset, shuffle=True, batch_size=args.test_batch_size, 
