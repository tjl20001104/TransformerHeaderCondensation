...
def main(args, **kwargs):
    ...
    for file in ['pic', 'loss', 'src', 'data', 'model', 'similarity']:
...
if __name__ == '__main__':
    ...
    # 数据集参数
    parser.add_argument('-N_train', '--train_data_size', type = int, default = 1024) 
    parser.add_argument('-N_test', '--test_data_size', type = int, default = 256)
    ...
    # 网络超参数
    parser.add_argument('-train_bs', '--train_batch_size', type = int, default = 1024) 
    ...
    parser.add_argument('-dh', '--d_head', type = int, default = 64)
    ...
    parser.add_argument('-init_mean', '--init_mean', type = float, default = 0.0, help='所有线性层小初始化均值')
    parser.add_argument('-init_var_ff', '--init_var_ff', type = float, default = 1e-3, help='其他线性层小初始化方差')
    parser.add_argument('-init_var_attn', '--init_var_attn', type = float, default = 1e-2, help='注意力线性层小初始化方差')
    ...
    parser.add_argument('-scheduler', '--scheduler', type = str, default = None)
    ...
    parser.add_argument('-pae', '--print_acc_epoch', type = int, default = 20, help='每隔多少epoch输出一次acc')
    parser.add_argument('-plae', '--plot_loss_acc_epoch', type = int, default = 500, help='每隔多少epoch画一次loss和acc')
    parser.add_argument('-phse', '--plot_headoutput_similarity_epoch', type = int, default = 500, help='每隔多少epoch画一次headoutput_similarity')
    parser.add_argument('-phst', '--plot_headoutput_similarity_threshold', type = float, default = 0.8, help='对headoutput_similarity聚类的threshold')
