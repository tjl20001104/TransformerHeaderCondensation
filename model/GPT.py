...
class MultiHeadAttention(nn.Module):
    def __init__(self, args):
        ...
        self.W_Q = nn.Linear(args.d_model, args.d_head * args.n_heads, bias=False)
        nn.init.normal_(self.W_Q.weight, mean=args.init_mean, std=args.init_var_attn)

        self.W_K = nn.Linear(args.d_model, args.d_head * args.n_heads, bias=False)
        nn.init.normal_(self.W_K.weight, mean=args.init_mean, std=args.init_var_attn)

        self.W_V = nn.Linear(args.d_model, args.d_head * args.n_heads, bias=False)
        nn.init.normal_(self.W_V.weight, mean=args.init_mean, std=args.init_var_attn)

        self.fc = nn.Linear(args.n_heads * args.d_head, args.d_model, bias=False)
        nn.init.normal_(self.fc.weight, mean=args.init_mean, std=args.init_var_ff)
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):
        ...
        Q = self.W_Q(input_Q).view(batch_size, -1, self.n_head, self.d_head).transpose(1, 2)  # Q: [batch_size, n_heads, len_q, d_k]
        K = self.W_K(input_K).view(batch_size, -1, self.n_head, self.d_head).transpose(1, 2)  # K: [batch_size, n_heads, len_k, d_k]
        V = self.W_V(input_V).view(batch_size, -1, self.n_head, self.d_head).transpose(1, 2)  # V: [batch_size, n_heads, len_v(=len_k), d_v]

        attn_mask = attn_mask.unsqueeze(1).repeat(1, self.n_head, 1, 1)  # attn_mask : [batch_size, n_heads, seq_len, seq_len]

        # scores : [batch_size, n_heads, len_q, len_k]
        attn = torch.matmul(Q, K.transpose(-1, -2)) / np.sqrt(self.d_head)
        ...
        qkv_out = qkv.transpose(1, 2).reshape(batch_size, -1, self.n_head * self.d_head)  # context: [batch_size, len_q, n_heads * d_v]
...
class PoswiseFeedForwardNet(nn.Module):
    def __init__(self, args):
        ...
        nn.init.normal_(self.fc[0].weight, mean=args.init_mean, std=args.init_var_ff)
        nn.init.normal_(self.fc[2].weight, mean=args.init_mean, std=args.init_var_ff)
...
class myGPT(nn.Module):
    def __init__(self, args, device):
        ...
        nn.init.normal_(self.projection.weight, mean=args.init_mean, std=args.init_var_ff)
