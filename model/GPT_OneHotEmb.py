...
class Embedding(nn.Module):
    def __init__ (self, args, device):
        ...
        # tgt_emb使用独热编码
        self.tgt_emb = nn.Embedding(args.vocab_size, args.vocab_size)
        self.tgt_emb.weight.data = torch.eye(args.vocab_size)
...
class myGPT_OneHotEmb(nn.Module):
    def __init__(self, args, device):
        super(myGPT_OneHotEmb, self).__init__()

        self.device = device
        args.d_model = args.vocab_size  # 由于使用独热编码，所以d_model的大小等于vocab_size
